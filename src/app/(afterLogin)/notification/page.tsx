...
export default function Page() {
  const { notifications, isFetchingNextPage,fetchNextPage,hasNextPage } = useNotificationQuery();
  ...
  return (
    <main className="flex flex-col gap-2 items-center justify-between p-6">
      <div className="flex w-full justify-center flex-col items-center">
        {notifications?.map((a: notificationMessage, index) => {
          const isLastElement = index == notifications.length - 1;
          return (
          <NotificationItem ref={isLastElement ? lastElementRef : null} key={index} title={a.title} content={a.content} />
          );
        })}
